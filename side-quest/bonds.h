 ...
 *   dot(a, b) = a.t*b.t + a.d*b.d + a.m*b.m + a.q*b.q
 ...
 *                Bit 5: C direction (1 if b.d > a.d, 0 if b.d < a.d)
...
static inline uint8_t classify_bond_directed(coord4 from, coord4 to) {
    ...
    if (from.d != to.d) {
        mask |= BOND_C;
        if (to.d > from.d) dirs |= 2;  /* +C: forward spatial */
    }
    if (from.m != to.m) {
        mask |= BOND_R;
 ...
 *   P+W   = filter   C+W   = maybe    R+W   = scan
 *   P+C+R = foldl    P+C+W = map      P+R+W = scanl
