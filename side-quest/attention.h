 ...
 *   R = resistance (connection indirectness, increases with distance)
 *   C = capacitance (information content of the token)
 *
 ...
 *
 * Capacitance is derived from the coordinate itself:
 *   - More active dimensions (nonzero) = more information = higher C
 *   - Silence (0,0,0,0) has C=0: no information, decays instantly
 *   - A vowel like 'a' (+1,+1,+1,+1) has C=4: maximum information
 *   - A glide like 'l' (0,0,0,+1) has C=1: minimal information
 *
 * At the word level (walks of prime coordinates), coordinates
 * with large absolute values carry more semantic weight.
 * "Einstein" at (3, -1, 5, 2) has higher C than "the" at (1, 0, 0, 0).
 *
...
static inline AttentionResult lattice_attention_range(
    ...
    result.context.d = (int8_t)roundf(dim_vals[1]);
 ...
 * by Manhattan distance in the full 4D space.
 *
 * Algorithm:
 *   1. Compute Manhattan distance from query to every position.
 ...
 *
 * Why Manhattan distance for selection?
 *   - Manhattan = L1 = sum of per-dimension distances.
 *   - Matches token_capacitance (also L1 norm).
 *   - Cheap to compute: four subtractions and four absolute values.
 *   - On a discrete lattice, Manhattan distance counts the minimum
 *     number of unit steps between two points.  It's the natural
 *     distance metric for integer coordinates.
 *
...
static inline AttentionResult sparse_attention(const coord4 *walk,
    ...
    /*
     * Step 1: compute Manhattan distance from query to every position.
     *
    ...
    for (int i = 0; i < n; i++) {
        ...
        int dd = (int)q.d - (int)walk[i].d; if (dd < 0) dd = -dd;
 ...
 * coordinate-proximate 25% of past positions, and computes
 * 4-head dimensional attention over them.
 *
...
static inline AttentionResult causal_sparse_attention(
    ...
    /* Compute distances to all past positions */
    int dists[1024];
    int indices[1024];
